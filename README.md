...
- 用于真实余额查询的 DeepSeek API 密钥，或用于 plan 用量查询的智谱 GLM Coding Plan API 密钥
- 如需查询 Codex 额度，本机需要已通过 Codex 登录，生成 `$CODEX_HOME/auth.json` 或 `~/.codex/auth.json`

## 安全说明

- API 密钥只通过 `KeychainCredentialStore` 存储在 macOS 密钥串中。
- Codex provider 优先只读本机 Codex auth 文件，不修改、不删除，也不会复制到 UserDefaults；密钥串仅作为手工 fallback。
- 保存后不会再明文展示已保存的 key。API 密钥的配置、更换和删除都在本地控制台中完成。
- 测试只使用假 key，不需要真实 DeepSeek、智谱或 Codex 账号。
- 不要把真实 API 密钥、Codex access token、session token 或 account id 写进源码、文档、日志、截图或 shell history。
...
1. 从 GitHub Releases 下载 `API-Inquiry-v0.3.11.dmg` 和 `API-Inquiry-v0.3.11.dmg.sha256`。
2. 校验下载文件：
...
- 从控制台保存 key 后清空输入框，并将 key 存入密钥串。
- key 已配置后，控制台只显示 `已配置` 或 `Configured`，不会展示已保存 key。
...
- `开机自启` 操作用于切换登录时自动启动，启用后模块颜色会变化。
- 最近更新时间会跟随系统的 12 小时制或 24 小时制设置。
- 默认语言为 `Auto`，会根据 macOS 首选语言显示中文或英文。
- Console Home 可手动切换 `自动 / 中文 / English`，切换后菜单栏详情和 Console 会同步更新。
- 中文 UI 使用紧凑额度标签：`5 时` 和 `1 周`，避免影响菜单栏和详情页视觉宽度。
- 手动刷新与自动刷新使用同一条刷新路径。
...
- Codex 详情页展示 5h 和 Week 两个剩余额度窗口。
- Console Home 的 OpenAI/Codex 卡片展示手动重置额度，可单独刷新并查看明细。
...
- 智谱 GLM Coding Plan 会在展开面板显示重置时间，并在 Console Home 显示计划下次重置时间。
