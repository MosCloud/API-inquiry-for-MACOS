...
- large balance value, such as `¥68.65 CNY`, with only the numeric amount enlarged, all balance parts set at medium weight, and the currency symbol/code kept at the previous scale
- small status line:
  - available
