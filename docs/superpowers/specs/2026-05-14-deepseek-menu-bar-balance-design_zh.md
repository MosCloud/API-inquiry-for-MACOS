...
菜单栏标题以 `DS ¥68.6` 这种形式为基础。展开面板显示极简的 DeepSeek 余额视图，包含手动刷新、状态、最近刷新时间、API Key 设置、外部控制台入口和退出。详细用量图表、本地 DeepSeek 控制台、多供应商界面都延后处理。
...
- 第一阶段交付可在本机运行的源码项目，后续再考虑打包、签名和分发。
...
如果本机没有存储 DeepSeek API Key，菜单栏使用紧凑的未配置状态，例如 `DS Setup`。
...
API Key 只在用户输入时可见。保存后，常规 UI 不再以明文展示 API Key。
...
后续优化展示可以在余额前加入供应商图标，例如图标加 `¥68.65`。第一版应在视图模型中为这种展示方式留出空间，但不需要提供设置开关。
