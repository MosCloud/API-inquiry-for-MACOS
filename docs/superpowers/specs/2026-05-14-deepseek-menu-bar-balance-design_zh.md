...
- 小号状态行：
  ...
  - 正在刷新
  - 刷新失败
- 最近一次成功刷新时间，格式跟随用户系统的 12 小时制或 24 小时制偏好
- 刷新按钮，位置易触达但视觉上保持安静
- 次要操作：
  - 通过 `AutoStart` 切换开机自启
  ...
  - 删除 API Key
  - 退出
- 底部操作使用三个等宽的图标加文字模块：`AutoStart`、`Console` 和 `Quit`
- 底部操作文字统一使用紧凑的 13 pt medium 字重样式，按钮最小高度收敛到 32 pt
- `AutoStart` 在开机自启启用时改变颜色，关闭后回到普通样式
...
- 绘制 DeepSeek 标识时必须保留源图宽高比，不能强行放入正方形区域。
