...
菜单栏标题以 DeepSeek template 图标加 `¥68.6` 这类紧凑余额文本为基础。展开面板显示极简的 DeepSeek 余额视图，标题区域使用 DeepSeek logo，包含手动刷新、状态、最近刷新时间、API Key 设置、外部控制台入口和退出。详细用量图表、本地 DeepSeek 控制台、多供应商界面都延后处理。
...
- 使用单色 template DeepSeek 图标替代 `DS` 文字前缀。
- CNY 尽量使用人民币符号。
- 菜单栏显示一位小数，节省空间。
