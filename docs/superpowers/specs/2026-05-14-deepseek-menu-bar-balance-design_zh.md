...
菜单栏标签以动态渲染的 DeepSeek template 图像加 `¥68.6` 这类紧凑余额文本为基础。展开面板显示极简的 DeepSeek 余额视图，标题区域使用可随浅色/深色外观自适应的单色 DeepSeek logo，包含手动刷新、状态、最近刷新时间、API Key 设置、外部控制台入口和退出。详细用量图表、本地 DeepSeek 控制台、多供应商界面都延后处理。
...
- 菜单栏图标略高于金额文字，贴近常见 macOS 状态栏项目比例。
- CNY 尽量使用人民币符号。
- 菜单栏显示一位小数，节省空间。
...
- 顶部 logo 尺寸收紧，让余额成为视觉焦点
- 大号余额，例如 `¥68.65 CNY`，仅放大数字部分，货币符号和货币代码保持原有尺度
- 小号状态行：
  - 可用
  - 余额不足
  - 未配置
