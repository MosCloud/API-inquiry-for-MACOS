...
API Inquiry 是一个原生 macOS 菜单栏应用，用于查询 DeepSeek API 账号余额，并在本地查看 DeepSeek 用量导出。它会将一个 DeepSeek API Key 保存到 macOS Keychain，每 5 分钟自动刷新官方余额 API，支持手动刷新，在菜单栏显示当前余额，并支持将 DeepSeek Usage 官方 zip 导出包导入本地控制台。
...
- 导入的用量数据只以归一化后的本地记录保存在 Application Support 下。API Inquiry 不会把原始导出文件复制进应用数据目录。
...
- 控制台可以导入 DeepSeek Usage 官方 zip 导出包，并展示本地总览、模型汇总和明细记录。
- 清空用量数据只删除本地用量记录，不会删除 API Key。
...
- DeepSeek Usage 官方 zip 导入，并保留兼容 CSV 兜底
- 本地用量总览、模型汇总和明细记录
- 本地 `.app` bundle 生成
- 自定义 macOS 应用图标生成与打包
- 详情面板中的开机自启控制
