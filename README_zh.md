...
API Inquiry 是一个原生 macOS 菜单栏应用，用于查询 DeepSeek API 账号余额，并在本地查看 DeepSeek 用量导出。它会将一个 DeepSeek API Key 保存到 macOS Keychain，每 5 分钟自动刷新官方余额 API，支持手动刷新，在菜单栏显示当前余额，并支持将 DeepSeek Usage CSV 导入本地控制台。
...
- 导入的用量数据只以归一化后的本地记录保存在 Application Support 下。API Inquiry 不会把原始 CSV 文件复制进应用数据目录。
...
- 控制台可以导入 DeepSeek Usage CSV，并展示本地总览、模型汇总和明细记录。
